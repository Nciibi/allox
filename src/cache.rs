...
const LARGE_STASH_HUGE_CAP_BYTES: usize = 64 * 1024 * 1024;
const LARGE_STASH_HUGE_THRESHOLD: usize = 4 * 1024 * 1024;
...
impl ThreadCache {
    ...
    fn note_alloc(&mut self, class: usize) {
        ...
        self.pending.bytes_in += CLASSES_RUNTIME[class] as u64;
    ...
    fn note_free(&mut self, class: usize) {
        ...
        self.pending.bytes_out += CLASSES_RUNTIME[class] as u64;
        if self.pending.ops >= FLUSH_OPS {
            self.publish();
        }
    }
    ...
    fn drift_gate_open(&self) -> bool {
        if self.tier_cached_bytes == 0 {
            return self.cached_bytes > self.budget / DRIFT_GATE_DIV;
        }
        self.small_cached_bytes() > self.budget / DRIFT_GATE_DIV
            || self.tier_cached_bytes > self.budget.saturating_mul(2) / DRIFT_GATE_DIV
    }
