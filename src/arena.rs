...
pub(crate) unsafe fn medium_table_set(
    ...
    if let Some(start) = big_page_range(base, pages) {
        let value = master as usize | MEDIUM_TABLE_TAG;
        for i in 0..pages as usize {
            BIG_MAP[start + i].store(value, Ordering::Release);
        }
    } else {
        debug_assert!(false, "medium table set outside reservation");
    }
}
...
pub(crate) unsafe fn large_table_clear(base: *mut u8, pages: u32) {
    if let Some(start) = big_page_range(base, pages) {
        ...
        debug_assert!(false, "large table clear outside reservation");
    }
}
...
pub(crate) unsafe fn medium_table_clear(base: *mut u8, pages: u32) {
    if let Some(start) = big_page_range(base, pages) {
        ...
        debug_assert!(false, "medium table clear outside reservation");
    }
}
