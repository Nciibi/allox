...
unsafe impl GlobalAlloc for Allox {
    ...
    unsafe fn realloc(&self, p: *mut u8, layout: core::alloc::Layout, new_size: usize) -> *mut u8 {
        ...
        // can (ecs-style doubling). Gated to exactly the large tier
...
#[cfg(test)]
mod large_grow_tests {
    use super::*;
    use crate::page::PAGE_SIZE;

    /// Direct mremap-path coverage with a hand-built legacy region (never
    /// arena-contained, so `try_grow` must attempt the kernel path): contents
    /// survive whether growth happens in place or relocates, and the header
    /// stays dispatchable afterwards. Raw `mremap`/`map_any` syscalls are
    /// unsupported under Miri — the integration growth tests cover the
    /// alloc-copy-free fallback there.
    #[test]
    #[cfg_attr(miri, ignore)]
    fn legacy_grow_preserves_contents() {
        unsafe {
            let mapped = 2 * PAGE_SIZE;
            let base = sys::map_any(mapped);
            assert!(!base.is_null());
            let user = 65536usize;
            let ret = align_up(base as usize + LARGE_HEADER_SIZE, MIN_ALIGN);
            assert!(ret + user <= base as usize + mapped);
            for i in 0..user {
                *(ret as *mut u8).add(i) = (i % 251) as u8;
            }
            let hdr = (ret - LARGE_HEADER_SIZE) as *mut LargeHeader;
            (*hdr).magic = LARGE_MAGIC;
            (*hdr).mapped_size = mapped;
            (*hdr).base = base;
            // Grow 64 KiB -> 400 KiB (forces a bigger mapping).
            let nuser = 400000usize;
            let q = try_grow_large_in_place(base, mapped, nuser, MIN_ALIGN);
            assert!(!q.is_null());
            for i in 0..user {
                assert_eq!(*(q.add(i)), (i % 251) as u8, "lost byte at {}", i);
            }
            assert!(usable_size(q) >= nuser);
            core::ptr::write_bytes(q, 0x5A, nuser);
            // Release via the (possibly relocated) header.
            let h2 = (q as usize - LARGE_HEADER_SIZE) as *mut LargeHeader;
            assert_eq!((*h2).magic, LARGE_MAGIC);
            sys::unmap((*h2).base, (*h2).mapped_size);
        }
    }

    /// Shrink/same-size requests refuse (other paths own them) with the
    /// original mapping untouched.
    #[test]
    #[cfg_attr(miri, ignore)]
    fn grow_refuses_shrink() {
        unsafe {
            let mapped = 4 * PAGE_SIZE;
            let base = sys::map_any(mapped);
            assert!(!base.is_null());
            let ret = align_up(base as usize + LARGE_HEADER_SIZE, MIN_ALIGN);
            ...
            (*hdr).mapped_size = mapped;
            (*hdr).base = base;
            assert!(try_grow_large_in_place(base, mapped, 1000, MIN_ALIGN).is_null());
            for i in 0..100000usize {
                assert_eq!(*((ret as *mut u8).add(i)), (i % 251) as u8);
            }
            sys::unmap(base, mapped);
        }
    }

    /// Arena-owned regions refuse (hole accounting can't survive a
    /// relocation); the slice parks back cleanly afterwards.
    #[test]
    #[cfg_attr(miri, ignore)]
    #[cfg(all(unix, feature = "std"))]
    fn grow_refuses_arena_region() {
        unsafe {
            let (base, fresh) = crate::arena::commit(2);
            assert!(!base.is_null());
            assert!(fresh);
            let mapped = 2 * page::PAGE_SIZE;
            let q = try_grow_large_in_place(base, mapped, mapped + 1, MIN_ALIGN);
            assert!(q.is_null());
            crate::arena::release(base, 2);
        }
    }
}
